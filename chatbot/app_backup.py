...
The UI also allows the user to select a response persona.
...
if not LLM_API_KEY:
    st.error(
        "LLM_API_KEY غير موجود. تأكد من إضافته في ملف .env"
    )
    st.stop()
...
# ---------------------------------------------------------------------------
# User information + Persona selection
# ---------------------------------------------------------------------------
...
selected_persona_name = PERSONAS[selected_persona]["name"]

st.caption(
    f"أسلوب الرد الحالي: {selected_persona_name}"
)
...
if question:
    ...
    with st.spinner(
        ...
        # 3. Ask Gemini using the selected persona
        answer = ask_llm(
            question,
            context,
            llm_client,
